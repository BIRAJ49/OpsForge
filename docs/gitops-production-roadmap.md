...
2. **Activate the real GitOps source of truth.** Remote `BIRAJ49/OpsForge-GitOps` still has the production-only migration scaffold; the current local working tree adds the canonical multi-environment state. Merge and protect it, produce new health-capable digests, configure its external dependencies, and complete the safe root cutover before setting `ENABLE_GITOPS_STAGING_PROMOTION=true`. Do not maintain copied platform manifests in two repositories after cutover.
...
The application workflow can only open or update a staging digest-promotion pull request, and that path is deliberately feature-flagged. Keep it disabled until the staging overlay and its validation are ready. Production remains a separate, reviewed GitOps change that promotes the same already-attested digest; the application workflow must never target it.
