...
10. **Review the exact infrastructure plan.** The current protected job safely applies the plan it creates, but environment approval authorizes the commit before that plan exists. Adopt HCP Terraform, Atlantis, or encrypted private plan storage when reviewers must approve the exact rendered production plan.
...
OpsForge/                         # Product source
  backend/
  frontend/
  .github/workflows/              # PR CI and image release only
...
- application delivery has no cluster credentials and Terraform never runs as part of an application release;
